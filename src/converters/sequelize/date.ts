/**
 * Date helpers for Sequelize: year/month combos and date ranges.
 */
...
export function tryHandleYearMonth(a: ODataNode, b: ODataNode): SequelizeWhere | null {
...
export function tryHandleYear(node: ODataNode): SequelizeWhere | null {
...
export function tryHandleDateRange(a: ODataNode, b: ODataNode): SequelizeWhere | null {
  const aIsLower =
  ...
  const ops = getSequelizeOps();
  const startSym = a.type === NodeType.GREATER_OR_EQUALS_EXPRESSION ? ops.gte : ops.gt;
  const endSym = b.type === NodeType.LESSER_OR_EQUALS_EXPRESSION ? ops.lte : ops.lt;
  return { [aField]: { [startSym]: startValue, [endSym]: endValue } };
}
